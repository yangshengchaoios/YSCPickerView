...
- (void)awakeFromNib {
    ...
    [self.datePicker setMinimumDate:[dateFormatter dateFromString:@"1930-01-01"]];
    [self.datePicker setMaximumDate:[NSDate date]];

    
    //点击半透明关闭选择器
    UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(doneButtonClicked:)];
    [self addGestureRecognizer:tapGesture];
    
    //点击完成按钮关闭选择器
    [self.doneButton addTarget:self action:@selector(doneButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
}
- (void)dataPickerValueChanged:(id)sender {
    if (self.pickerType < YSCPickerTypeAddress) {
        if (self.selectingBlock) {
            self.selectingBlock([self.datePicker date]);
        }
    }
}
...
- (void)showPickerView:(id)initObject {
    ...
    [UIView animateWithDuration:DurationOfAnimation animations:^{
        ...
        if (YSCPickerTypeAddress == self.pickerType) {
            ...
            if (initRegion && [initRegion isKindOfClass:[RegionModel class]]) {
                //选中province
            ...
            if ((nil == initDate) || (! [initDate isKindOfClass:[NSDate class]])) {
